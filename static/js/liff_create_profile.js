...
agreeTerms.change(function () {
    submitBtn.prop('disabled', (!agreeTerms.prop('checked')));
});
...
$("#submitBtn").click(function () {
    ...
    const gender = $('#male').prop('checked') ? 0 : 1;
    const school = $('#school').val();
    const department = $('#department').val();

    if (name === '' || city === '' || district === '' || city == null || district == null || birthday == '' || birthday == null) {
        alert('請填寫完整資料');
        return false;
    }

    try {
        $("#divPlaceloader").hide(100);
        $.ajax({
